...
class FoldingNet_noCov(ShapeNetGraph, FoldingNetDecoder2D, Adam):
    ...
    def encoder(self):
        ...
        cc = self.cc
        N = self.data_shape()[-2]
        cc.silence(["n_offset", "Cov"])
        cc.comment_bar("Encoder")
        self.XKrelu("X", "X1", 64).XKrelu("X1", "X2", 64).XKrelu("X2", "X3", 64).LocalMaxPooling("X3", "X3m").XKrelu(
            "X3m", "X4", 128
...
if __name__ == "__main__":
    ...
    FoldingNet_noCov(NETWORK_NAME).main()
